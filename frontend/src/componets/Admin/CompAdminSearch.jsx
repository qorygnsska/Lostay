...
export default function CompAdminSearch(props) { //props: 상위요소에게서 받아온 유산
  ...
  const keyHandler = (event) => {  //엔터키 누르면 #btn_search 클릭 실행
  ...
  const submitHandler = (event) => { //엔터키 누르면 제출(page reload) 방지
  ...
  const nullHandler = () => {  //모든 값 가져오기?
    //console.log('input_nothing');
  }

  const searchHandler = () => {  //검색 필터링
    //console.log('input_something');
  }

  return (
    <>
      <Form className="comp--admin--search--container" onKeyUp={keyHandler} onSubmit={submitHandler} >
        <Form.Control
          type="search"
          placeholder="Search"
          className="me-2"
        ...
        />
        <Button id="btn_search" onClick={clickHandler} variant="outline-primary" size="sm"><CiSearch size="24" /></Button>
