...
for i in range(1,6):
    driver.get(f'https://www.yeogi.com/domestic-accommodations?sortType=RECOMMEND&keyword=%EC%A0%9C%EC%A3%BC%EB%8F%84&page={i}&personal=1&checkIn=2024-10-21&checkOut=2024-10-22&reservationActive=STAY&category=2&freeForm=true')

    hotel_list = driver.find_elements(By.CSS_SELECTOR, 'a.gc-thumbnail-type-seller-card')
    links = [ele.get_attribute('href') for ele in hotel_list]
    ...
    for hotel_id, link in enumerate(links):
        ...
        try:
            ...
            os.makedirs(hotel_list_folder, exist_ok=True)  # 호텔 폴더 생성
            ...
            hotel_name = get_hotel_name[0].text if get_hotel_name else ''
            ...
            hotel_level = get_hotel_level[0].text if get_hotel_level else ''
            ...
            hotel_info = get_hotel_info[0].text if get_hotel_info else ''

            # 호텔 주소 가져오기
            get_hotel_location = driver.find_elements(By.CSS_SELECTOR, '#location > div.css-1rdjshf > ul > div > p')
            hotel_location = get_hotel_location[0].text if get_hotel_location else ''
            ...
            # 호텔 데이터 삽입 쿼리 출력
            thumbnail_path = os.path.join(thumbnail_folder, hotel_thumbnail_name).replace('\\', '/')
            images_path = [os.path.join(images_folder, img_name).replace('\\', '/') for img_name in hotel_images_name]
            images_path_str = ', '.join(images_path)
            hotel_service_str = ', '.join(hotel_service)
            hotel_touristPlace_str = ', '.join(hotel_touristPlace)
            ...
            for room_idx, room in enumerate(room_list):
                ...
                # 객실 썸네일 및 이미지 가져오기
                room_images_url = room.find_elements(By.CSS_SELECTOR, 'img.css-10f94ee')
                for idx, url in enumerate(room_images_url):
                    if idx == 0:
                        room_thumbnail = url.get_attribute('src')
                    room_images.append(url.get_attribute('src'))
                ...
                room_info_str = ', '.join(room_info)

                room_query = f"INSERT INTO ROOM VALUES(DEFAULT, '{hotel_name}', '{room_title}', '{room_thumbnail}', '{room_price}', '{number_person}', '{room_discount}', '{room_service_str}', '{room_info_str}');"

                with open('room_insert_queries.txt', 'a', encoding='utf-8') as file:
                    file.write(room_query + '\n')  # 쿼리와 줄 바꿈 추가

        except Exception as e:
            print(f"호텔 정보 수집 중 오류 발생: {e}")
