...
# 해당 지역의 호텔 검색
destination = ["부산,한국", "서울,한국"]
sort_type_list=["RECOMMENDED", "PRICE_LOW_TO_HIGH", "PRICE_HIGH_TO_LOW", "REVIEW_RELEVANT", "PROPERTY_CLASS"]

hotel_link_list = [] #해당 지역의 호텔을 검색해서 정렬타입별로 긁어모아 담을 컨테이너


for sort_type in sort_type_list:

    html = requests.get("https://kr.hotels.com/Hotel-Search?destination="+destination[0]+"&regionId=&flexibility=0_DAY&d1=&startDate=2024-11-13&d2=&endDate=2024-11-14&adults=2&rooms=1&theme=&userIntent=&semdtl=&useRewards=false&sort="+sort_type)
    ...
    print(f'this_sort_type: {sort_type}')
    print(f'sizeOf_hotel_link_list_bySelect: {len(hotel_link_list_bySelect)}')
    hotel_link_list.extend(hotel_link_list_bySelect) #이번 정렬 방식으로 긁어온 링크 리스트를 추가



for index, anchor in enumerate(hotel_link_list, start=1):
